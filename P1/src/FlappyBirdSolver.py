...
class FlappyBirdSolver:

    def __init__(self, iter_for_ind=10, norm_reward=False) -> None:
        self.best_ind = None
        self.best_fit = -np.inf
        self.iters_for_ind = iter_for_ind
        self.envs = [gym.make("FlappyBird-v0") for _ in range(self.iters_for_ind)]
        self.norm_reward = norm_reward
        self.pool = multiprocessing.Pool(processes=self.iters_for_ind)
        self.max_fitnesses = []
        self.min_fitnesses = []
        self.avg_fitnesses = []
        self.times_elapsed = []
    ...
    def crossover(self, ind1: MLP, ind2: MLP, pcross: float) -> tuple[MLP, MLP]:
        ...
        if random.random() < pcross:
            ...
            return MLP(layers=[12, 16, 16, 2], chromosome=new_ind1), MLP(layers=[12, 16, 16, 2], chromosome=new_ind2)
        return ind1, ind2
    ...
    def evolve(self,
               ...
               plt_show=False, save_best=True, save_best_path=f"results/best_flappybird_{time.strftime('%Y-%m-%d_%H-%M')}",
        ...
        Args:
            ...
            save_best_path (_type_, optional): _description_. Defaults to f"results/best_flappybird_{time.strftime('%Y-%m-%d_%H-%M')}".
