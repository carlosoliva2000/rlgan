...
def run (model: MLP, norm_reward=False, visualize=False):
    env = gym.make("FlappyBird-v0", render_mode="human" if visualize else None)
...
def test():
    ...
    #     )
        PREFIX_PATH = "results/best_flappybird_2024-02-06_16-29"  # "results/best_lunarlander_2024-02-06_10-44"
        EPOCHS = 10
        VISUALIZE = True
        ...
        for i in range(EPOCHS):
            result=run(model, visualize=VISUALIZE)
        ...
        try:
            flappybird_solver = FlappyBirdSolver()
            flappybird_solver.avg_fitnesses = np.load(os.path.join(PREFIX_PATH, "avg_fitnesses.npy"))
            flappybird_solver.max_fitnesses = np.load(os.path.join(PREFIX_PATH, "max_fitnesses.npy"))
            flappybird_solver.min_fitnesses = np.load(os.path.join(PREFIX_PATH, "min_fitnesses.npy"))
            flappybird_solver.times_elapsed = np.load(os.path.join(PREFIX_PATH, "times_elapsed.npy"))
            flappybird_solver.plot_fitness(show_min=False, show_avg=False)
